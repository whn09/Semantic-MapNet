...
def run_astar(episode):
    ...
    AGENT_HEIGHT = 0.88
    AGENT_RADIUS = 0.18
    house_floor_threshold = {'2azQ1b91cZZ': [1.5],
                             '8194nk5LbLH': [1.2],
                             'EU6Fwq7SyZv': [-1.3, 2.0],
    ...
    if house_floor_threshold[house]:
        level = bisect.bisect(house_floor_threshold[house], episode['start_position'][1])
    else:
        level = 0
    ...
    pathfinder = Astar(navmap, observed_map, heuristic=distance_map,
                       init_heading=start_heading, goals=goals)
...
for file in tqdm(files):
    ...
    res = pool.map(run_astar, jsonfile['episodes'][:32])  # TODO only use 32 episodes
